...
public interface PaymentDetailDao {
    ...
    static String loadAll = "select DetailId as detail_id,StationId as station_id,CreateDate " +
            "as create_date,Balance as balance,BalanceAmount as balance_amount,BananceType as balance_type," +
            "Comment as balance_comment from paymentDetails";
    /**
     * 创建一条收支记录
     */
    static String CreatePayment = "insert into paymentDetails(StationId,CreateDate,Balance,BalanceAmount,BananceType,Comment)" +
    ...
    static String UpdatePayment = "update paymentDetails set StationId=#{station_id}" +
            ...
            ",BananceType=#{balance_type},Comment=#{balance_comment}  where DetailId=#{detail_id}";
    ...
    static String searchByTime = "select DetailId as detail_id,StationId as station_id,CreateDate as create_id,Balance as balance,BalanceAmount as balance_amount,BananceType as balance_type,Comment as balance_comment from paymentDetails where  CreateDate BETWEEN #{0} AND #{1}";

    /**
     * 按收入支出以及日期条件查询出收支明细
     */
    static String searchByAll = "select DetailId as detail_id,StationId as station_id,CreateDate as create_id,Balance as balance,BalanceAmount as balance_amount,BananceType as balance_type,Comment as balance_comment from paymentDetails  where Balance=#{0} AND CreateDate BETWEEN #{1} AND #{2} ";

    /**
     * 按收入支出条件查询出收支明细
     */
    static String search = "select DetailId as detail_id,StationId as station_id,CreateDate as create_id,Balance as balance,BalanceAmount as balance_amount,BananceType as balance_type,Comment as balance_comment from paymentDetails  where Balance=#{balance} ";
